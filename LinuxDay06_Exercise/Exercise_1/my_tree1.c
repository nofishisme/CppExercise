...
void printBranch(struct dirent* pdirent, DIR* dirp, const char *filepath) {
    printPreGraph();
    //检查下一个不是"."和".."的目录项目是否为空
    ...
    struct stat statbuf;
    char *filename = get_file_path(filepath, pdirent->d_name);
...
void my_tree_recrusion(const char* dirname) {
    ...
    while ((pdirent = readdir(dirp)) != NULL) {
        ...
        if (pdirent->d_type == DT_DIR) {
            //打印分支和文件项
            printBranch(pdirent, dirp, dirname);
            count_dir++;
