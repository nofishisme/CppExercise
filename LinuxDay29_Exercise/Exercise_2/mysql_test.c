...
int main()
{
    ...
    const char *stmt_str = "update t_user set name = 'lisi' where id = 2";
    res = mysql_query(mysql, stmt_str);
    const char *stmt_str2 = "select id, name, email from t_user where id > 0";
    res = mysql_query(mysql, stmt_str2);
    if(res != 0){
        printf("查询数据表失败！\n");
        return -1;
    }
