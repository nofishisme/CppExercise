...
int handle_new_connection(chat_connect * connection_set, int epoll_fd, int socket_fd){
    ...
    for(int i = 0; i < CONNECTION_LIMIT; i++){
        if(connection_set[i].is_alive == false) {
            ...
            connection_set[i].the_time = time(NULL);
            break;
        }
    }
...
//处理超时，第三个参数为设定的超时时间
void handle_overtime_connection(chat_connect *connection_set, 
                                int epoll_fd, 
                                int time_limit){
    char message[64];
    sprintf(message, "overtime %d s, disconnect.\n", time_limit);
    time_t now_time = time(NULL);
    for(int i = 0; i< CONNECTION_LIMIT; i++) {
        if(connection_set[i].is_alive && 
           now_time - connection_set[i].the_time >= time_limit) {
            send(connection_set[i].conection_fd, message, strlen(message), 0);
            printf("client disconnect over %d s, fd: %d\n", 
                   time_limit, 
            ...
            connection_set[i].is_alive = false;
            //从监听队列删除该连接
            epoll_delfd(epoll_fd, connection_set[i].conection_fd);
...
int main(int argc, char** argv)
{
    ...
    while(1){
        ...
        //监听就绪
        int ready = epoll_wait(epoll_fd, event_set, 2, 1000);
        if (ready == -1) {
            printf("epoll_wait\n");
            break;
        } else if (ready == 0){
        ...
        handle_overtime_connection(connection_set, epoll_fd, 30);
    }
    close(socket_fd);
