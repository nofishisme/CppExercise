...
void client(){
    ...
    ERROR_CHECK(write_fd, -1 ,"client read_fd open.\n");
    int read_fd = open("1.pipe", O_RDONLY);
    ERROR_CHECK(read_fd, -1 ,"client write_fd open.\n");

    const char *initString = "client is connected.\n";
    write(write_fd, initString, strlen(initString));
